...
function stemNL(n){
  ...
  else if(n.endsWith('onen')){
    // sperziebonen → sperzieboon, persimonen → persimoon
    n=n.slice(0,-4)+'oon';
  } else if(n.endsWith('zen')){
    // frambozen → framboos, abrikozen → abrikoos
    n=n.slice(0,-3)+'s';
...
function categorizeIngredient(name){
  ...
  // Controleer eerst op exacte match of langere keywords (specifieker) om false positives
  // te voorkomen (bv. "tomatenpuree" moet naar Sauzen, niet naar Groenten via "tomaat").
...
// Synoniemen: verschillende spellingen → één canonieke naam in de boodschappenlijst.
const SYNONYMS={
...
// Geeft de boodschappenlijst terug als [{category, items:[{key,name,qty,unit,cat}]}].
// Items zijn objecten zodat de modal hoeveelheden kan aanpassen en opslaan.
...
function ShoppingListModal({userSlug,initialDate,onClose}){
  ...
  // Voeg handmatig toegevoegde items samen met de gegenereerde lijst
  const list=useMemo(()=>{
  ...
  function copy(){
    try{navigator.clipboard.writeText(buildText());setCopied(true);setTimeout(()=>setCopied(false),2000);}catch{}
  }
  function shareWhatsApp(){
    window.open('https://wa.me/?text='+encodeURIComponent('🛒 Boodschappenlijst\n\n'+buildText()),'_blank');
  }
